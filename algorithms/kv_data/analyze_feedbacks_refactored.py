...
def find_most_popular_products(mentions, top_count=3):
    temporary_mentions = mentions.copy()
    popular_products = {}
    for time in range(top_count):
        popular_name = ""
        max_count = 0
        for product_name, count in temporary_mentions.items():
            if count > max_count:
                popular_name = product_name
                max_count = count
        popular_products[popular_name] = max_count
        temporary_mentions.pop(popular_name)
    print(f'popular_products: {popular_products}')
    print(f'temporary_mentions: {temporary_mentions}')


def find_least_popular_products(mentions, min_count=3):
    temporary_mentions = mentions.copy()
    unpopular_products = {}
    for time in range(min_count):
        unpopular_name = ""
        min_count = next(iter(temporary_mentions.values()))  # Takes first value from temporary mentions
        for product_name, count in temporary_mentions.items():
            if count < min_count:
                unpopular_name = product_name
                min_count = count
        unpopular_products[unpopular_name] = min_count
        temporary_mentions.pop(unpopular_name)
    print(f'unpopular_products: {unpopular_products}')
    print(f'temporary_mentions: {temporary_mentions}')
